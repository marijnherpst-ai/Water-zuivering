...
const STAPPEN = [
  ['1. Kraanwater komt binnen', 'Uw gewone kraanwater stroomt eerst door een voorfilter, die grof vuil, zand en chloor tegenhoudt.'],
  ['2. Het osmosemembraan filtert', 'Onder lichte druk perst het systeem het water door een membraan van 0,0001 micron — ruim fijn genoeg om PFAS, medicijnresten, microplastics en zware metalen tegen te houden.'],
  ['3. Nafilter voor de laatste smaak', 'Een laatste filtertrap verfijnt de smaak, zodat er alleen zuiver, lekker drinkwater overblijft.'],
  ['4. Direct uit uw eigen kraan', 'Geen waterflessen, geen sjouwen: schoon water rechtstreeks uit de kraan, elke dag opnieuw.'],
];
  ...
  return (
    <>
      ...
      <main>
        ...
        {/* WAT DOET ONZE WATERZUIVERAAR PRECIES */}
        <section className="relative overflow-hidden">
          <div className="relative max-w-6xl mx-auto px-6 py-16 md:py-24 grid lg:grid-cols-2 gap-12 items-center">
            <div className="relative rounded-3xl overflow-hidden border border-edge aspect-[4/3] order-2 lg:order-1">
              <Image src="/assets/img/systeem-slim-105mm.png" alt="Osmose waterzuiveraar, slechts 10,5 cm breed, geïnstalleerd onder het aanrecht" fill sizes="(min-width: 1024px) 50vw, 100vw" className="object-contain p-6 bg-white" />
            </div>
            <div className="order-1 lg:order-2">
              <span className="text-xs font-bold uppercase tracking-widest text-amber-dark">Onze oplossing</span>
              <h2 className="mt-3 font-display text-2xl md:text-3xl font-extrabold tracking-tight">Wat doet onze waterzuiveraar precies?</h2>
              <p className="mt-4 text-dim leading-relaxed">
                Onze osmose waterzuiveraar wordt compact onder uw aanrecht geïnstalleerd en filtert al uw kraanwater in vier stappen — zonder dat u er iets voor hoeft te doen.
              </p>
              <div className="mt-6 space-y-4">
                {STAPPEN.map(([titel, uitleg]) => (
                  <div key={titel} className="flex gap-3">
                    <span className="shrink-0 flex items-center justify-center w-8 h-8 rounded-full bg-amber/15 text-amber-dark text-xs font-bold">{titel.slice(0, 1)}</span>
                    <div>
                      <p className="font-display font-bold text-ink text-sm">{titel.slice(3)}</p>
                      <p className="mt-1 text-sm text-dim leading-relaxed">{uitleg}</p>
                    </div>
                  </div>
                ))}
              </div>
              <Link href="/aanmelden" className="cursor-pointer mt-8 inline-flex items-center gap-2 rounded-full bg-amber px-7 py-3.5 text-sm font-bold text-ink hover:bg-amber-dark hover:text-white transition-colors shadow-lg shadow-amber/25">
