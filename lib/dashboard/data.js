...
function leesDetail(tekst) {
  if (!tekst) return [];
  try {
    const v = JSON.parse(tekst);
    return Array.isArray(v) ? v.filter((x) => x && typeof x.k === 'string') : [];
  } catch {
    return [];
  }
}
...
async function haalLeads(supabase, vanaf) {
  const rijen = await alles(() => supabase.from('lead_events').select('id,created_at,lead_type,page_path,utm_source,utm_medium,utm_campaign,utm_term,utm_content,gclid,fbclid,journey,journey_days,journey_detail,consent').gte('created_at', `${vanaf}T00:00:00Z`).order('created_at', { ascending: false }).order('id'));
  return rijen.map((l) => ({
    id: l.id,
    ...
    journey: l.journey || '',
    dagen: l.journey_days == null ? null : Number(l.journey_days),
    detail: leesDetail(l.journey_detail),
...
export async function haalMetaData(supabase) {
  ...
  const [camp, adRijen, uurRijen, doelgroepRijen, bereik, status, leads, aanvragen, kwaliteit, account] = await Promise.all([
    alles(() => supabase.from('meta_campaign_daily').select('day,campaign_id,campaign,status,spend,impressions,reach,clicks,leads,landing_views').gte('day', vanaf).order('day').order('campaign_id')),
    alles(() => supabase.from('meta_ad_daily').select('day,ad_id,ad,adset_id,adset,campaign_id,campaign,spend,impressions,clicks,leads,landing_views').gte('day', vanaf).order('day').order('ad_id')),
    ...
    haalLeads(supabase, vanaf),
    haalAanvragen(supabase),
    supabase.from('meta_ad_quality').select('ad_id,ad,adset,campaign,quality,engagement,conversion').then((r) => r.data || []),
    supabase.from('meta_account_status').select('account_status,disable_reason,balance,amount_spent,currency,updated_at').eq('id', 1).maybeSingle().then((r) => r.data || null),
  ]);

  const campagnes = camp.map((r) => ({ day: r.day, campaign_id: r.campaign_id, campaign: r.campaign, status: r.status, impressions: r.impressions, clicks: r.clicks, cost: Number(r.spend), conversions: Number(r.leads), landing_views: r.landing_views }));
  const zoekwoorden = adRijen.map((r) => ({
    ...
    impressions: r.impressions, clicks: r.clicks, cost: Number(r.spend), conversions: Number(r.leads), landing_views: r.landing_views,
  }));
  const uren = uurRijen
    .map((r) => ({
  ...
  return {
    bron: 'facebook', vandaag, campagnes, zoekwoorden, zoektermen: [], leads: metKanaal(leads.filter((l) => l.type !== 'giveaway'), 'facebook'), alleLeads: leads, uren, doelgroep, bereik, kwaliteit, account,
    aanvragen, bijgewerkt: status?.last_sync || null, syncStatus: status, demo: false,
  };
}

export async function haalSamenData(supabase) {
  const [google, facebook] = await Promise.all([haalDashboardData(supabase), haalMetaData(supabase)]);
  return { bron: 'samen', vandaag: google.vandaag, google, facebook, leads: google.alleLeads, aanvragen: google.aanvragen, demo: false };
}
