...
function maakAdvies({ zoekwoorden, zoektermen, campagnes, inst, fb }) {
  const advies = [];
  ...
  for (const z of zoekwoorden) {
    if (z.oordeel.status === 'slecht') {
      advies.push({
        ...
        titel: fb ? `Zet de advertentie "${z.naam}" op pauze` : `Zet het zoekwoord "${z.naam}" op pauze`,
        ...
        ref: { niveau: fb ? 'advertentie' : 'zoekwoord', naam: z.naam, groep: z.groep, campagne: z.campagne, kosten: z.kosten, leads: z.leads },
      });
    } else if (z.oordeel.status === 'goed' && z.leads >= 1) {
      advies.push({
        id: `meer-${z.key}`, soort: 'meer', niveau: 'goed', kosten: z.kosten,
        titel: fb ? `Geef de advertentie "${z.naam}" meer budget` : `Geef "${z.naam}" meer ruimte`,
...
export function bouwOverzicht(data, periodeId, inst = STANDAARD_INSTELLINGEN) {
  ...
  const zoekwoorden = sorteer(kwRijen.map((z) => metOordeel(z, siteLeadsVoor(leadsNu, (l) => (fb ? l.groep && slug(l.groep) === slug(z.naam) : l.term && l.term === norm(z.naam))), inst)));

  const groepRijen = groepeer(kwNu, (r) => String(r.ad_group_id), (r) => ({ naam: r.ad_group, campagne: r.campaign }));
  const groepen = sorteer(
    groepRijen.map((g) => {
      const leads = siteLeadsVoor(leadsNu, (l) => {
        if (fb) return l.term && slug(l.term) === slug(g.naam);
  ...
  const advies = maakAdvies({ zoekwoorden, zoektermen, campagnes, inst, fb });
  ...
  const tijden = tijdenAnalyse(data, inst, fb);
  const orde = { slecht: 0, goed: 1 };
...
function tijdenAnalyse(data, inst, fb) {
  ...
  [...groepen].filter((g) => g.slechtste).sort((x, y) => y.slechtste.verlies - x.slechtste.verlies).slice(0, 4).forEach((g) => {
    ...
    advies.push({
      ...
      titel: fb ? `Laat "${g.naam}" minder zien in de ${d.naam.toLowerCase()} (${d.tijd})` : `Bied minder voor "${g.naam}" in de ${d.naam.toLowerCase()} (${d.tijd})`,
  ...
  [...groepen].filter((g) => g.beste && g.beste.leads >= 3 && g.beste.cpl <= inst.doelCpl).sort((x, y) => y.beste.leads - x.beste.leads).slice(0, 3).forEach((g) => {
    ...
    advies.push({
      ...
      titel: fb ? `Laat "${g.naam}" meer zien in de ${d.naam.toLowerCase()} (${d.tijd})` : `Bied meer voor "${g.naam}" in de ${d.naam.toLowerCase()} (${d.tijd})`,
