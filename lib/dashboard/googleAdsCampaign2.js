...
export async function pasBudgetAan(budgetId, bedragMicros, customerId = CUSTOMER_ID) {
  const ops = [{ campaignBudgetOperation: { update: {
    resourceName: `customers/${customerId}/campaignBudgets/${budgetId}`,
    amountMicros: String(bedragMicros),
  }, updateMask: 'amount_micros' } }];
  const results = await gaqlMutate(ops, { customerId });
  return { ok: true, results };
}

export async function pasZoekwoordBodAan(criterionResourceName, bodMicros, customerId = CUSTOMER_ID) {
  const ops = [{ adGroupCriterionOperation: { update: {
...
// Hergebruikte asset-resources van de bestaande "Water-zuivering" Performance Max-campagne
const HERGEBRUIK_LOGO = `customers/${CUSTOMER_ID}/assets/423631479152`;
const HERGEBRUIK_MARKETING_IMAGE = `customers/${CUSTOMER_ID}/assets/423632622182`;
const HERGEBRUIK_SQUARE_MARKETING_IMAGE = `customers/${CUSTOMER_ID}/assets/423732026566`;

const PFAS_PMAX_HEADLINES = [
  'PFAS Check', 'PFAS Water', 'Gratis Check',
  'PFAS in Kraanwater?', 'Check PFAS Gratis', 'PFAS Uit Water Halen', 'Osmose Tegen PFAS',
  'Zuiver Water Zonder PFAS', 'Verwijdert 99% PFAS', 'Gratis PFAS Check',
];
const PFAS_PMAX_LONG_HEADLINES = [
  'Check gratis hoeveel PFAS er in uw kraanwater zit en ontdek de oplossing',
  'Osmose waterzuiveraar verwijdert tot 99% van PFAS uit uw kraanwater',
];
const PFAS_PMAX_DESCRIPTIONS = [
  'Doe de gratis PFAS-check voor uw regio.',
  'Vakkundig geinstalleerd, snel geleverd.',
  'Persoonlijk advies op maat.',
];
const PFAS_SEARCH_THEMES = [
  'pfas water', 'pfas kraanwater', 'pfas waterfilter', 'pfas verwijderen', 'pfas drinkwater', 'pfas check', 'pfas nederland',
];

function datumPlusDagen(dagen) {
  const d = new Date();
  d.setDate(d.getDate() + dagen);
  return d.toISOString().slice(0, 10);
}

// Performance Max (PFAS Ad) laat de hele itemgroep pas draaien na de eerste beoordeling
// van de itemgroep - dat kan 1-3 dagen duren, te laat voor een kortdurende trendcampagne.
// Search-advertenties draaien vrijwel meteen (review loopt op de achtergrond door), dus
// deze Search-versie vervangt 'm zodat de campagne nu al kan vertonen op de PFAS-piek.
const PFAS_SEARCH_GROEPEN = [
  {
    name: 'PFAS Check', finalUrl: `${BASE}/pfas-check`,
    keywords: ['pfas check', 'pfas kraanwater check', 'gratis pfas check', 'pfas checken'],
    headlines: ['PFAS Check', 'Gratis Check', 'Check PFAS Gratis', 'Gratis PFAS Check', ...GEDEELDE_HEADLINES].slice(0, 15),
    descriptions: [...PFAS_PMAX_DESCRIPTIONS, ...GEDEELDE_DESCRIPTIONS].slice(0, 4),
  },
  {
    name: 'PFAS Waterfilter', finalUrl: `${BASE}/pfas-check`,
    keywords: ['pfas waterfilter', 'pfas verwijderen', 'pfas uit water halen', 'pfas filter'],
    headlines: ['PFAS Uit Water Halen', 'Osmose Tegen PFAS', 'Zuiver Water Zonder PFAS', ...GEDEELDE_HEADLINES].slice(0, 15),
    descriptions: [...PFAS_PMAX_DESCRIPTIONS, ...GEDEELDE_DESCRIPTIONS].slice(0, 4),
  },
  {
    name: 'PFAS Drinkwater', finalUrl: `${BASE}/pfas-check`,
    keywords: ['pfas drinkwater', 'pfas water', 'pfas kraanwater', 'pfas nederland'],
    headlines: ['PFAS Water', 'PFAS in Kraanwater?', 'PFAS Check', ...GEDEELDE_HEADLINES].slice(0, 15),
    descriptions: [...PFAS_PMAX_DESCRIPTIONS, ...GEDEELDE_DESCRIPTIONS].slice(0, 4),
  },
];

export async function maakPfasSearchCampagne() {
  const cid = CUSTOMER_ID;
  PFAS_SEARCH_GROEPEN.forEach((g) => {
    assertLen(g.headlines, 30, `Kop (${g.name})`);
    assertLen(g.descriptions, 90, `Beschrijving (${g.name})`);
  });
  ...
  ops.push({ campaignBudgetOperation: { create: {
    resourceName: `customers/${cid}/campaignBudgets/-1`,
    name: `PFAS Ad Search budget ${Date.now()}`,
  ...
  const results = await gaqlMutate(ops, { customerId: cid });
  return { ok: true, results, groepen: PFAS_SEARCH_GROEPEN.length, aantalZoekwoorden: allKeywords.length };
}

export async function maakPfasPmaxCampagne() {
  const cid = CUSTOMER_ID;
  assertLen(PFAS_PMAX_HEADLINES, 30, 'PMax kop');
  assertLen(PFAS_PMAX_LONG_HEADLINES, 90, 'PMax lange kop');
  assertLen(PFAS_PMAX_DESCRIPTIONS, 90, 'PMax beschrijving');
  ...
  ops.push({ campaignBudgetOperation: { create: {
    resourceName: `customers/${cid}/campaignBudgets/-1`,
    name: `PFAS PMax budget ${Date.now()}`,
    amountMicros: '30000000',
    deliveryMethod: 'STANDARD',
    explicitlyShared: false,
  } } });
  ...
  const agRN = `customers/${cid}/assetGroups/-5`;

  // Fase 1: alle tekst-assets aanmaken (nog niets koppelen)
  let tmpId = -10;
  const headlineRNs = PFAS_PMAX_HEADLINES.map((text) => {
    const rn = `customers/${cid}/assets/${tmpId--}`;
    ops.push({ assetOperation: { create: { resourceName: rn, textAsset: { text } } } });
    return rn;
  });
  const longHeadlineRNs = PFAS_PMAX_LONG_HEADLINES.map((text) => {
    const rn = `customers/${cid}/assets/${tmpId--}`;
    ops.push({ assetOperation: { create: { resourceName: rn, textAsset: { text } } } });
    return rn;
  });
  const descriptionRNs = PFAS_PMAX_DESCRIPTIONS.map((text) => {
    const rn = `customers/${cid}/assets/${tmpId--}`;
    ops.push({ assetOperation: { create: { resourceName: rn, textAsset: { text } } } });
    return rn;
  });
  const businessNameRN = `customers/${cid}/assets/${tmpId--}`;
  ops.push({ assetOperation: { create: { resourceName: businessNameRN, textAsset: { text: 'Water-zuivering' } } } });

  // Fase 2: itemgroep aanmaken
  ops.push({ assetGroupOperation: { create: {
    resourceName: agRN,
    name: 'PFAS Itemgroep',
    campaign: campaignRN,
    status: 'ENABLED',
    finalUrls: ['https://www.water-zuivering.nl/pfas-check'],
  } } });

  // Fase 3: alles koppelen
  headlineRNs.forEach((rn) => ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: rn, fieldType: 'HEADLINE' } } }));
  longHeadlineRNs.forEach((rn) => ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: rn, fieldType: 'LONG_HEADLINE' } } }));
  descriptionRNs.forEach((rn) => ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: rn, fieldType: 'DESCRIPTION' } } }));
  ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: HERGEBRUIK_MARKETING_IMAGE, fieldType: 'MARKETING_IMAGE' } } });
  ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: HERGEBRUIK_SQUARE_MARKETING_IMAGE, fieldType: 'SQUARE_MARKETING_IMAGE' } } });

  // Brand Guidelines is enabled: bedrijfsnaam + logo horen ALLEEN op campagneniveau,
  // niet (ook) op itemgroep-niveau - anders botsen de twee koppelingen.
  ops.push({ campaignAssetOperation: { create: { campaign: campaignRN, asset: businessNameRN, fieldType: 'BUSINESS_NAME' } } });
  ops.push({ campaignAssetOperation: { create: { campaign: campaignRN, asset: HERGEBRUIK_LOGO, fieldType: 'LOGO' } } });

  PFAS_SEARCH_THEMES.forEach((text) => {
    ops.push({ assetGroupSignalOperation: { create: {
      assetGroup: agRN,
      searchTheme: { text },
    } } });
  });
...
export async function verwijderAssetGroupAssets(resourceNames, customerId = CUSTOMER_ID) {
  const ops = resourceNames.map((rn) => ({ assetGroupAssetOperation: { remove: rn } }));
  const results = await gaqlMutate(ops, { customerId });
  return { ok: true, results };
}

export async function zetCampagneStatus(campaignId, status, customerId = CUSTOMER_ID) {
  const ops = [{ campaignOperation: { update: {
...
// Leads-Search-1 stond met 36 uiteenlopende zoekwoorden in één generieke advertentiegroep
// op één advertentie/landingspagina - lage kwaliteitsscore (1/10), maar 10% impression share.
// Zelfde STAG-aanpak als Leads-Search-2: kleine, thema-specifieke groepen met bijpassende landingspagina.
const CAMPAGNE1_ID = '24282768664';
const OUDE_GROEP_ID_SEARCH1 = '199135165583';

const STAG_GROEPEN_SEARCH1 = [
  {
    name: 'Waterfilter Kraan', finalUrl: `${BASE}/waterfilter-kraan`,
    keywords: ['waterfilter kraan', 'filter voor kraanwater', 'kraan filter', 'kraan water filter', 'waterfilter voor kraan', 'kraanwaterfilter', 'waterfiltersysteem kraan', 'kraanfilter', 'keukenkraan filter'],
    h1: 'Waterfilter voor Kraan', h2: 'Kraanwaterfilter Kopen',
    d1: 'Waterfilter voor uw kraan, vakkundig geinstalleerd. Vraag vrijblijvend advies.',
  },
  {
    name: 'Osmose Systeem', finalUrl: `${BASE}/osmosesysteem`,
    keywords: ['osmose installatie', 'osmose water filter', 'osmose apparaat kopen', 'osmose systeem kopen', 'osmosesysteem'],
    h1: 'Osmose Systeem Kopen', h2: 'Osmose Installatie Thuis',
    d1: 'Osmose systeem voor kraakhelder drinkwater thuis. Vakkundig geinstalleerd.',
  },
  {
    name: 'Waterzuiveraar Drinkwater', finalUrl: `${BASE}/waterzuiveraar-drinkwater`,
    keywords: ['waterfilter drinkwater', 'drinkwaterfilter', 'drinkwater filter', 'waterzuiveraar drinkwater', 'gefilterd water'],
    h1: 'Waterzuiveraar Drinkwater', h2: 'Drinkwaterfilter Kopen',
    d1: 'Waterzuiveraar voor zuiver drinkwater, direct uit de kraan. Vraag advies aan.',
  },
  {
    name: 'Waterzuivering Thuis', finalUrl: `${BASE}/waterzuivering-voor-thuis`,
    keywords: ['waterzuiveraar voor thuis', 'beste waterfilter voor thuis', 'waterzuivering thuis', 'waterzuivering voor thuis'],
    h1: 'Waterzuivering voor Thuis', h2: 'Waterzuiveraar voor Thuis',
    d1: 'Waterzuivering voor thuis regelen? Wij adviseren en installeren vakkundig.',
  },
  {
    name: 'Waterfiltersysteem', finalUrl: `${BASE}/waterfiltersysteem`,
    keywords: ['waterfiltersysteem', 'waterfilter systemen', 'waterfilter apparaat'],
    h1: 'Waterfiltersysteem Kopen', h2: 'Waterfiltersysteem Thuis',
    d1: 'Waterfiltersysteem voor thuis, vakkundig geinstalleerd. Vraag offerte aan.',
  },
  {
    name: 'Waterzuivering Kopen', finalUrl: `${BASE}/waterzuiveringsinstallatie`,
    keywords: ['waterzuivering kopen', 'waterzuiveringsfilter', 'waterzuiveraar kopen', 'waterzuiveringsinstallatie drinkwater'],
    h1: 'Waterzuiveringsinstallatie', h2: 'Waterzuivering Kopen',
    d1: 'Waterzuiveringsinstallatie op maat, vakkundig geplaatst. Vraag advies aan.',
  },
  {
    name: 'Waterfilter Kopen', finalUrl: `${BASE}/winkel`,
    keywords: ['waterfilter', 'aqua waterfilter', 'beste waterfilter'],
    h1: 'Waterfilter Kopen', h2: 'Beste Waterfilter',
    d1: 'Waterfilter kopen? Vergelijk onze systemen en ontvang persoonlijk advies.',
  },
];

export async function herstructureerSearch1NaarStags() {
  const cid = CUSTOMER_ID;
  STAG_GROEPEN_SEARCH1.forEach((g) => {
    const headlines = [g.h1, g.h2, ...GEDEELDE_HEADLINES].slice(0, 15);
    const descriptions = [g.d1, ...GEDEELDE_DESCRIPTIONS].slice(0, 4);
    assertLen(headlines, 30, `Kop (${g.name})`);
    assertLen(descriptions, 90, `Beschrijving (${g.name})`);
  ...
  const ops = [];
  let tmpId = -300;
  const campaignRN = `customers/${cid}/campaigns/${CAMPAGNE1_ID}`;

  ops.push({ adGroupOperation: { update: {
    resourceName: `customers/${cid}/adGroups/${OUDE_GROEP_ID_SEARCH1}`,
    status: 'PAUSED',
  }, updateMask: 'status' } });
  ...
  const results = await gaqlMutate(ops, { customerId: cid });
  return { ok: true, results, groepen: STAG_GROEPEN_SEARCH1.length, aantalZoekwoorden: allKeywords.length };
}
