...
export async function maakPfasPmaxCampagne() {
  ...
  let tmpId = -10;
  PFAS_PMAX_HEADLINES.forEach((text) => {
    const id = tmpId--;
    const rn = `customers/${cid}/assets/${id}`;
    ops.push({ assetOperation: { create: { resourceName: rn, textAsset: { text } } } });
    ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: rn, fieldType: 'HEADLINE' } } });
  });
  PFAS_PMAX_LONG_HEADLINES.forEach((text) => {
    const id = tmpId--;
    const rn = `customers/${cid}/assets/${id}`;
    ops.push({ assetOperation: { create: { resourceName: rn, textAsset: { text } } } });
    ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: rn, fieldType: 'LONG_HEADLINE' } } });
  });
  PFAS_PMAX_DESCRIPTIONS.forEach((text) => {
    const id = tmpId--;
    const rn = `customers/${cid}/assets/${id}`;
    ops.push({ assetOperation: { create: { resourceName: rn, textAsset: { text } } } });
    ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: rn, fieldType: 'DESCRIPTION' } } });
  });
  const businessNameId = tmpId--;
  const businessNameRN = `customers/${cid}/assets/${businessNameId}`;
  ops.push({ assetOperation: { create: { resourceName: businessNameRN, textAsset: { text: 'Water-zuivering' } } } });
  ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: businessNameRN, fieldType: 'BUSINESS_NAME' } } });

  ops.push({ assetGroupAssetOperation: { create: { assetGroup: agRN, asset: HERGEBRUIK_LOGO, fieldType: 'LOGO' } } });
  ...
  // Brand Guidelines vereist bedrijfsnaam + logo ook op campagneniveau gekoppeld
