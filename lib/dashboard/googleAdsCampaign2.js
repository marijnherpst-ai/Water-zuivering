...
export async function testMinimaleCampagne() {
  ...
  const budgetResults = await gaqlMutate(budgetOps, { customerId: cid });
  const budgetRN = budgetResults[0].campaignBudgetResult.resourceName;
  ...
  const campaignResults = await gaqlMutate(campaignOps, { customerId: cid });
