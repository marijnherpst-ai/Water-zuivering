...
export function demoData(vandaag) {
    ...
    ZOEKWOORDEN.forEach(([gid, naam, kd, cpc, kans], idx) => {
      ...
      for (let c = 0; c < conv; c++) {
        ...
        leads.push({
          ...
          isAds: true, kanaal: 'google', journey: r() < 0.25 ? 'facebook>google' : 'google', term: norm(naam), groep: groep.slug, camp: slug(camp.naam),
        });
      }
    });
  ...
  organisch.forEach(([type, pagina], i) => {
    ...
    leads.push({
      id: `org-${i}`, day, uur: 9 + i, tijd: `${day}T1${i}:20:00.000Z`, type, pagina, isAds: false, kanaal: null, journey: '', term: '', groep: '', camp: '',
    });
  });
...
const FB_CAMPAGNES = [
  { id: '501', naam: 'Zuiveraar weggeven' },
  { id: '502', naam: 'Verkoop water-zuivering' },
  { id: '503', naam: 'Sittard regio' },
];

const FB_ADSETS = [
  { id: '601', camp: '501', naam: 'Nederland 25 tot 65' },
  { id: '602', camp: '501', naam: 'Lookalike websitebezoekers' },
...
const FB_ADS = [
  ['601', 'Afbeelding: winactie waterzuiveraar', 3200, 0.021, 0.16, 0.16],
  ['601', 'Video: win een waterzuiveraar', 2600, 0.028, 0.14, 0.2],
  ['602', 'Afbeelding: winactie waterzuiveraar', 1800, 0.018, 0.19, 0.12],
  ['602', 'Reel: kraan open, zuiver water', 1500, 0.013, 0.22, 0.05],
...
export function demoMeta(vandaag) {
  ...
  for (let i = 59; i >= 0; i--) {
    ...
    FB_ADS.forEach(([adsetId, ad, weergaven, ctr, cpc, kans], idx) => {
      ...
      const imp = Math.round(weergaven * 0.4 * groei * (0.7 + r() * 0.6));
      const klikken = Math.max(0, Math.round(imp * ctr * (0.8 + r() * 0.4)));
      const kosten = Math.round(klikken * cpc * (0.9 + r() * 0.2) * 100) / 100;
      let conv = 0;
      for (let k = 0; k < klikken; k++) if (r() < kans * (camp.id === '501' ? 1 : 0.55) * WEEKDAG_FACTOR[weekdag] * 0.9) conv++;
      zoekwoorden.push({
        day, campaign_id: camp.id, ad_group_id: adset.id, criterion_id: idx + 1, campaign: camp.naam, ad_group: adset.naam, keyword: ad,
        impressions: imp, clicks: klikken, cost: kosten, conversions: conv,
      });
      const t = totaal.get(camp.id);
      t.imp += imp; t.klik += klikken; t.kosten += kosten; t.conv += conv;

      for (let c = 0; c < conv; c++) {
        if (camp.id === '501' ? r() > 0.5 : r() > 0.7) continue;
        const uurNr = kies(FB_UUR.map((_, h) => fbConvUur(h)), r);
        const verkoop = camp.id !== '501';
        leads.push({
          id: `fb-${day}-${idx}-${c}`, day, uur: uurNr, tijd: `${day}T${String(uurNr).padStart(2, '0')}:${String(Math.floor(r() * 60)).padStart(2, '0')}:00.000Z`,
          type: verkoop ? ['contact', 'besparing', 'contact'][Math.floor(r() * 3)] : 'giveaway',
          pagina: verkoop ? ['/', '/besparing', '/waterzuivering-voor-thuis'][Math.floor(r() * 3)] : '/actie/win-een-waterzuiveraar',
          isAds: true, kanaal: 'facebook', journey: r() < 0.3 ? 'google>facebook' : 'facebook',
    ...
    FB_CAMPAGNES.forEach((c) => {
      ...
      campagnes.push({ day, campaign_id: c.id, campaign: c.naam, status: 'ACTIVE', impressions: t.imp, clicks: t.klik, cost: Math.round(t.kosten * 100) / 100, conversions: t.conv, conv_value: 0 });
  ...
  return {
    bron: 'facebook', vandaag, campagnes, zoekwoorden, zoektermen: [], leads, uren: uren.filter((x) => x.day >= grens30), doelgroep, bereik,
