...
function voegTouchToe(touches, kanaal, ts) {
  ...
  return [...touches, { k: kanaal, t: ts }].slice(-MAX_TOUCHES);
}
...
export function onthoudLanding() {
  ...
  if (heeftBetaald) memory.touches = voegTouchToe(memory.touches || [], kanaalVan(nu.betaald), nu.ts);
}
...
function samenvoegen(oud, nieuw) {
  let uit = [];
  [...oud, ...nieuw].sort((a, b) => a.t - b.t).forEach((x) => { uit = voegTouchToe(uit, x.k, x.t); });
  return uit;
}
