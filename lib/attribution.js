...
// Reads only the current page's own URL (query params + referrer) at the moment of
// submission. This is not stored on the visitor's device and does not persist across
// visits, so it is captured regardless of cookie consent - it just records which link
// this specific request/conversion arrived through, the same way a server access log
// would. Cross-session journey history below stays consent-gated.
function directeAttributie() {
  const nu = leesUrl();
  return {
    landing_page: nu.landing_page,
    referrer_host: nu.referrer_host,
    utm_source: nu.betaald.utm_source,
    utm_medium: nu.betaald.utm_medium,
    utm_campaign: nu.betaald.utm_campaign,
    utm_term: nu.betaald.utm_term,
    utm_content: nu.betaald.utm_content,
    gclid: nu.betaald.gclid,
    fbclid: nu.betaald.fbclid,
  };
}

export function getAttribution() {
  if (typeof window === 'undefined') return { consent: false };
  const direct = directeAttributie();
  if (!heeftToestemming()) return { consent: false, ...direct };
  bewaarAlsToegestaan();
  const data = lees() || memory;
  if (!data) return { consent: true, ...direct };
  const first = data.first || {};
  const paid = (data.paid && data.paid.betaald) || {};
  return {
    consent: true,
    landing_page: first.landing_page || direct.landing_page,
    referrer_host: first.referrer_host || direct.referrer_host,
    utm_source: paid.utm_source || direct.utm_source,
    utm_medium: paid.utm_medium || direct.utm_medium,
    utm_campaign: paid.utm_campaign || direct.utm_campaign,
    utm_term: paid.utm_term || direct.utm_term,
    utm_content: paid.utm_content || direct.utm_content,
    gclid: paid.gclid || direct.gclid,
    fbclid: paid.fbclid || direct.fbclid,
