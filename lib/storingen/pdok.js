...
// Generieke helper: zoekt vrije tekst (postcode, straat+plaats, etc.) op en
// geeft alleen de coördinaten van het beste resultaat terug — gebruikt om
// bronnen die zelf geen coördinaten meegeven toch op een kaart te tonen.
export async function geocodeCoordinaten(query, type) {
  const params = { q: query, rows: '1' };
  if (type) params.fq = `type:${type}`;
  const url = `https://api.pdok.nl/bzk/locatieserver/search/v3_1/free?${new URLSearchParams(params)}`;
